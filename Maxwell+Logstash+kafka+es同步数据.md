...
````
	bin/zookeeper-server-start.sh -daemon config/zookeeper.properties	=>启动zookeeper port:2181
	vim config/server.properties
	````
	broker.id=1 =>改了后报错 kafka无法启动
	````
	bin/kafka-server-start.sh config/server.properties	=>启动kafka；kafka默认日志路径，默认端口:9092
	bin/kafka-topics.sh --create --zookeeper 192.168.91.59:2181 --replication-factor 1 --partitions 1 --topic maxwell	=>创建maxwell topic
	bin/kafka-topics.sh --zookeeper 192.168.91.59:2181 --list	=>查看topic
	bin/kafka-console-consumer.sh --zookeeper 192.168.91.59:2181 --topic maxwell --from-beginning	=>启动消费者窗口查看maxwell数据
````

# maxwell

````
	bin/maxwell --user='search' --password='search@password1' --host='192.168.91.55' --producer=kafka --kafka.bootstrap.servers=192.168.91.59:9092	=> maxwell会在mysql中创建Maxwell数据库，确保数据库用户权限，maxwell会自动解析binlog日志，
...
````
	...
	/usr/..../logstash/bin/logstash -f /usr/..../config/conf.d/file_more_choose.conf	=>启动